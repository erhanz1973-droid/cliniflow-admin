...
app.use(cors({
  origin: true, // Allow all origins (or specify: ['http://localhost:3000', 'http://localhost:3001', ...])
  credentials: true,
  methods: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'OPTIONS'],
  allowedHeaders: ['Content-Type', 'Authorization', 'X-Patient-Token', 'X-Admin-Token'],
...
app.get("/api/patient/:patientId/travel", (req, res) => {
  ...
  if (data.formCompleted === undefined) {
    ...
    // ARRIVAL = Giriş (inbound), DEPARTURE = Dönüş (outbound)
    const hasHotel = data.hotel && data.hotel.checkIn && data.hotel.checkOut;
    const hasArrivalFlight = Array.isArray(data.flights) && 
      data.flights.some(f => {
        const type = String(f.type || "").toUpperCase();
        return (type === "ARRIVAL" || type === "OUTBOUND") && f.date; // ARRIVAL = Giriş
      });
    data.formCompleted = hasHotel && hasArrivalFlight;
  ...
  console.log(`[GET /travel/${patientId}] editPolicy in data:`, data?.editPolicy);
  console.log(`[GET /travel/${patientId}] editPolicy.flights:`, data?.editPolicy?.flights);
  console.log(`[GET /travel/${patientId}] flights in data:`, data?.flights);
  console.log(`[GET /travel/${patientId}] flights is array:`, Array.isArray(data?.flights));
  console.log(`[GET /travel/${patientId}] flights length:`, data?.flights?.length);
  if (Array.isArray(data?.flights) && data.flights.length > 0) {
    console.log(`[GET /travel/${patientId}] flight details:`, data.flights.map(f => ({
      id: f.id,
      type: f.type,
      from: f.from,
      to: f.to,
    ...
    console.log(`[GET /travel/${patientId}] WARNING: flights exists but is not an array:`, typeof data.flights, data.flights);
  }
...
app.post("/api/patient/:patientId/travel", (req, res) => {
  ...
  console.log(`[POST /travel/${patientId}] req.body.editPolicy:`, JSON.stringify(req.body?.editPolicy, null, 2));
  ...
  console.log(`[POST /travel/${patientId}] existing.editPolicy:`, JSON.stringify(existing?.editPolicy, null, 2));
  ...
  const payload = {
    ...
    // For flights: if provided in req.body, always use it (even if empty array).
    // This ensures patient can clear flights or add new ones.
    // Only preserve existing if flights key is completely missing from request
    flights: req.body?.hasOwnProperty('flights')
    ...
    // editPolicy: If provided in req.body, use it. Otherwise, keep existing (don't reset to defaults).
    // This ensures editPolicy set by admin or patient is preserved
    editPolicy: req.body?.editPolicy !== undefined 
      ? req.body.editPolicy 
      : (existing.editPolicy || {
          hotel: "ADMIN",
          flights: "ADMIN",
  ...
  console.log(`[POST /travel/${patientId}] Final payload editPolicy:`, JSON.stringify(payload.editPolicy, null, 2));
  ...
  // Ensure editPolicy is always an object
  if (!payload.editPolicy || typeof payload.editPolicy !== 'object') {
    console.log(`[POST /travel/${patientId}] WARNING: editPolicy missing or invalid, using defaults`);
    payload.editPolicy = {
      hotel: "ADMIN",
      flights: "ADMIN",
      airportPickup: "ADMIN",
  ...
  console.log(`[POST /travel/${patientId}] Final payload flights count:`, Array.isArray(payload.flights) ? payload.flights.length : 0);
  
  // Check if form is completed
  // Form is considered complete if:
  // - Hotel has checkIn and checkOut dates
  // - At least one arrival (inbound) flight exists
  // ARRIVAL = Giriş (inbound), DEPARTURE = Dönüş (outbound)
  const hasHotel = payload.hotel && payload.hotel.checkIn && payload.hotel.checkOut;
  const hasArrivalFlight = Array.isArray(payload.flights) && 
    payload.flights.some(f => {
      const type = String(f.type || "").toUpperCase();
      return (type === "ARRIVAL" || type === "OUTBOUND") && f.date; // ARRIVAL = Giriş
    });
  const isFormCompleted = hasHotel && hasArrivalFlight;
        ...
        clinicPatients.forEach(p => {
          ...
          if (patientId) {
            ...
            if (fs.existsSync(chatFile)) {
              try {
                const chatData = readJson(chatFile, { messages: [] });
                const messages = Array.isArray(chatData.messages) ? chatData.messages : [];
                messageCount += messages.length;
              } catch (e) {
                console.warn(`[SUPER_ADMIN] Error reading chat file for patient ${patientId}:`, e.message);
              }
            }
          }
          // Fallback: also check patient.messages (legacy)
